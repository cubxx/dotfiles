  ...
  config = function()
    ...
    local FileIcon = {
      init = function(self)
        ...
        if filetype == '' then
          ...
          self.icon, self.icon_hl, _ = icons.get('file', filepath)
        else
    ...
    local FileName = {
      provider = function(self)
        ...
        if buftype == '' then
          ...
          name = filepath and vim.fs.basename(filepath) or '[No name]'
        else
          name = buftype
        end
        return ' ' .. name
    ...
    local FilePath = {
      provider = function()
        ...
        local relpath = vim.fs.relpath(vim.fn.getcwd(), fullpath)
        return relpath == nil and fullpath or relpath
      end,
    }
    local BufferLine = utils.make_buflist({
      on_click = {
        callback = function(_, minwid, _, button)
          if button == 'r' then -- close on mouse right click
