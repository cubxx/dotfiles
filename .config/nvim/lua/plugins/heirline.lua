return {
  ...
  config = function()
    ...
    local get_buf_option = function(comp, name)
      return vim.api.nvim_get_option_value(name, { buf = comp.bufnr })
    end
    ...
    local FileName = {
      provider = function(self)
        ...
        if get_buf_option(self, 'buftype') ~= '' then
          return filepath
        end
        return filepath == '' and '[Untitled]' or vim.fn.fnamemodify(filepath, ':t')
      end,
      hl = function(self)
        return { fg = self.is_active and '' or 'gray' }
      end,
    }
    local FileFlag = {
      condition = function(self)
        ...
        elseif not get_buf_option(self, 'modifiable') or get_buf_option(self, 'readonly') then
          self.flag = ''
          if get_buf_option(self, 'buftype') == 'terminal' then
            self.flag = ''
          end
        end
        return self.flag
