...
on('VimEnter', {
  ...
  callback = function()
    ...
    local arg = args[#args]
    -- vim.notify('args ' .. vim.inspect(args), vim.log.levels.DEBUG)

    local stat = vim.uv.fs_stat(arg)
    if not stat then
      return
    end
    local dirpath = (stat.type == 'directory') and arg or vim.fs.dirname(arg)
    if dirpath ~= '' then
      vim.api.nvim_set_current_dir(dirpath)
    end
  end,
})
...
on('BufWritePre', {
  ...
  callback = function(e)
    ...
    if #clients > 0 then
      ...
      vim.cmd('silent! undojoin') -- merge undo node: code action + format
    end
